...
function useBuildVersion() {
  ...
  useEffect(() => {
    const meta = document.querySelector<HTMLMetaElement>('meta[name="x-build"]');
    if (meta?.content) {
...
function normaliseTags(tags: Array<string | { name?: string; slug?: string }> = []) {
...
const ProtonMailIcon: React.FC<IconProps> = ({
  size = 24,
  width,
  height,
  ...props
...
const CredlyIcon: React.FC<IconProps> = ({
  size = 24,
  width,
  height,
  ...props
    ...
    async function loadPosts() {
      try {
        ...
        const posts = Array.isArray((payload as { posts?: unknown })?.posts)
          ? ((payload as { posts: unknown[] }).posts as Array<Record<string, unknown>>)
          : [];
        ...
        const mapped = posts
          .map<BlogPost | null>((post) => {
            ...
            const tags = Array.isArray(post?.tags)
              ? normaliseTags(post.tags as Array<string | { name?: string; slug?: string }>)
              : [];
    ...
    <>
      ...
      <main>
            ...
            <Card className="about-card" radius="lg" shadow="sm" isBlurred>
              ...
              <CardBody className="about-card-body">
                <p>
                  ...
                  stakeholder collaboration. Passionate about leveraging Business
                  Intelligence tools to solve challenges in market forecasting,
                  supply chain optimization, and operational risk mitigation.
                </p>
                <p>
                  Certified in AWS Cloud, SCRUM, and data visualization (Tableau,
                  SQL), with a proven ability to deliver actionable insights for
        ...
        <section id="contact" className="section contact">
          <div className="container">
            <Card className="contact-panel" radius="lg" shadow="lg" isBlurred>
